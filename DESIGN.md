...
- 内容区：采用“Action Card + 数据区”结构；标题统一由顶栏 Header 提供
- 列表页（搜索 + 表格）：
  - 顶部：Action 区（Card），左侧搜索/筛选，右侧主操作（新增/导出）
  - 下方：表格直接渲染（不使用 Card 再包一层），表格自身带边框/圆角/表头底色区分
  - 底部：分页独立一行，与表格保持同宽对齐
...
- 表头：浅灰底（弱区分）+ `text-muted`；表头/表体行高统一偏舒适（避免过密）
- 表格容器：表格自身带边框/圆角；不再额外使用 Card 包裹表格
- 行 Hover：轻微背景色变化；行内操作按钮保持紧凑但可点击区域足够
